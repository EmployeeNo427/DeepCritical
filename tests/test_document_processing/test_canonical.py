...
def test_wide_hierarchy_uses_bounded_node_passes_and_preserves_child_order(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    document = _document()
    inputs = _fixture_inputs("pdf", document)
    original_children = [entry["$ref"] for entry in document["groups"][0]["children"]]
    width = 2_000
    new_references = [
        f"#/texts/{index}" for index in range(len(document["texts"]), 7 + width)
    ]
    document["texts"].extend(
        {
            "self_ref": reference,
            "label": "paragraph",
            "text": f"Wide hierarchy child {index}",
            "parent": {"$ref": "#/groups/0"},
        }
        for index, reference in enumerate(new_references)
    )
    explicitly_ordered = [
        new_references[-1],
        new_references[10],
    ...
    document["groups"][0]["children"].extend(
        {"$ref": reference} for reference in explicitly_ordered
    )
    expected_children = [
        *original_children,
        *explicitly_ordered,
        *(
            ...
            if reference not in set(explicitly_ordered)
        ),
    ]

    class CountingNodes(list[Any]):
        iterations = 0
    ...
    collect_native_nodes = canonical_module._collect_native_nodes
    nodes = CountingNodes(collect_native_nodes(document, [], inputs["docling_product"]))
    monkeypatch.setattr(
        canonical_module,
    ...
    blocks_by_id = {block.block_id: block for block in view.blocks}
    group = next(block for block in view.blocks if block.native_node_id == "#/groups/0")

    assert [
        blocks_by_id[child_id].native_node_id for child_id in group.child_block_ids
    ] == expected_children
    # A parent-by-node nested scan would iterate the collection once per parent.
    assert nodes.iterations < 25

    encoded = canonical_document_bytes(view)
    real_frozenset = frozenset
    child_lookup_sizes: list[int] = []

    def recording_frozenset(values: Any) -> frozenset[Any]:
        result = real_frozenset(values)
        child_lookup_sizes.append(len(result))
        return result
    ...
    assert load_canonical_document(encoded) == view
    assert len(expected_children) in child_lookup_sizes


def test_native_node_limit_counts_malformed_collection_entries_before_decoding(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    inputs = _fixture_inputs("pdf", _document())
    document = {
        collection: [] for collection in canonical_module._NATIVE_NODE_COLLECTIONS
    }
    document.update(
        {
            "body": {"children": []},
            "furniture": {"children": []},
            "texts": [None, "malformed"],
        }
    )
    inputs["docling_document"] = document
    monkeypatch.setattr(canonical_module, "MAX_CANONICAL_NATIVE_NODES", 1)

    def unexpected_collection(*_args: Any, **_kwargs: Any) -> None:
        raise AssertionError("native node decoding ran before the raw limit check")
    ...
    with pytest.raises(CanonicalDocumentError, match="NATIVE_NODE_LIMIT_EXCEEDED"):
...
@pytest.mark.parametrize(
    "mutate",
    [
        lambda document: document["body"].__setitem__("children", [None]),
        lambda document: document["furniture"].__setitem__(
            "children", [{"malformed": True}]
        ),
        lambda document: document["texts"].append({"children": [None]}),
        lambda document: document["texts"].append({"parent": None}),
        lambda document: document["tables"].append(
            {"data": {"table_cells": [{"ref": None}]}}
        ),
    ],
    ids=(
        "body-child",
...
def test_hierarchy_edge_limit_counts_every_raw_declaration_before_decoding(
    monkeypatch: pytest.MonkeyPatch,
    ...
    mutate(document)
    inputs["docling_document"] = document
    monkeypatch.setattr(canonical_module, "MAX_CANONICAL_HIERARCHY_EDGES", 0)
    ...
    with pytest.raises(
        CanonicalDocumentError,
        match="NATIVE_HIERARCHY_EDGE_LIMIT_EXCEEDED",
    ):
        build_canonical_document_view(**inputs)


def test_verified_model_reads_enforce_node_and_edge_limits(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    view = _fixture("pdf")
    payload = json.loads(canonical_document_bytes(view))
    ...
    with monkeypatch.context() as patch:
        ...
        with pytest.raises(ValidationError, match="NATIVE_NODE_LIMIT_EXCEEDED"):
            CanonicalDocumentView.model_validate(payload)
        with pytest.raises(InvalidCanonicalDocumentError):
            load_canonical_document(encoded)

    hierarchy_edges = sum(len(block.child_block_ids) for block in view.blocks)
    assert hierarchy_edges > 0
    with monkeypatch.context() as patch:
        patch.setattr(
            canonical_module,
            "MAX_CANONICAL_HIERARCHY_EDGES",
...
def test_duplicate_relationship_target_accounting_matches_multiset_semantics(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    counter_inputs: list[tuple[str, ...]] = []
    real_counter = canonical_module.Counter

    class RecordingCounter(real_counter):
        def __init__(self, values: tuple[str, ...]) -> None:
            counter_inputs.append(values)
            super().__init__(values)

    monkeypatch.setattr(canonical_module, "Counter", RecordingCounter)

    def reference_accounting(
        declared: tuple[str, ...],
        resolved_count: int,
        ...
        for declared_ref in declared:
            try:
                unresolved_index = remaining.index(declared_ref)
            except ValueError:
                pending.append(declared_ref)
            else:
                remaining.pop(unresolved_index)
        return tuple(pending[resolved_count:]), resolved_count == len(pending)

    rng = random.Random(20260812)
    vocabulary = ("a", "b", "c", "d")
    for _ in range(200):
        declared = tuple(rng.choice(vocabulary) for _ in range(rng.randrange(20)))
        unresolved = tuple(rng.choice(vocabulary) for _ in range(rng.randrange(20)))
        resolved_count = rng.randrange(20)
        assert canonical_module._declared_target_accounting(
            declared_target_refs=declared,
            resolved_target_count=resolved_count,
            unresolved_target_refs=unresolved,
        ) == reference_accounting(declared, resolved_count, unresolved)

    assert len(counter_inputs) == 200
    duplicate_refs = ("duplicate",) * 10_000
    assert canonical_module._declared_target_accounting(
        declared_target_refs=duplicate_refs,
        resolved_target_count=5_000,
        unresolved_target_refs=duplicate_refs[:5_000],
    ) == ((), True)
    assert counter_inputs[-1] == duplicate_refs[:5_000]
...
def test_builder_reports_malformed_native_structures_and_missing_anchors() -> None:
    ...
    assert "UNRESOLVED_NATIVE_REFERENCE" in {
        diagnostic.code for diagnostic in mixed_view.diagnostics
    }

    duplicate_furniture_root = _document()
    duplicate_furniture_root["furniture"]["children"] = [
        {"$ref": "#/groups/0"},
        {"$ref": "#/groups/0"},
    ]
    duplicate_furniture_view = build_canonical_document_view(
        **_fixture_inputs("pdf", duplicate_furniture_root)
    )
    assert duplicate_furniture_view.blocks
...
def test_builder_preserves_partial_and_scholarly_diagnostics() -> None:
    document = _document()
...
@pytest.mark.parametrize(
    ...
    [
        lambda document: (
            document["texts"][0].__setitem__("self_ref", "duplicate-node"),
            document["texts"][1].__setitem__("self_ref", "duplicate-node"),
        ),
        lambda document: (
            document["texts"][0].__setitem__("self_ref", "title-alias"),
            document["texts"][1].__setitem__("self_ref", "#/texts/0"),
        ),
    ],
    ids=("duplicate-declared-reference", "canonical-path-collision"),
)
def test_builder_rejects_native_identity_collisions_before_resolution(
    mutate: Callable[[dict[str, Any]], object],
) -> None:
    document = _document()
    mutate(document)
    inputs = _fixture_inputs("pdf", document)
    spans = inputs["content_span_set"]
    inputs["content_span_set"] = spans.model_copy(
        update={
            "spans": (
                spans.spans[0].model_copy(
                    update={
                        "representation_anchor": spans.spans[
                            0
                        ].representation_anchor.model_copy(
                            update={"node_id": "missing-before-span-resolution"}
                        )
                    }
                ),
                *spans.spans[1:],
...
def test_schema_version_constant_is_descriptive() -> None:
    ...
    assert MAX_CANONICAL_NATIVE_NODES == 100_000
    assert MAX_CANONICAL_HIERARCHY_EDGES == 500_000
