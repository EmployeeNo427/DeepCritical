...
Version 1 admits at most 100,000 raw native collection entries and 500,000 raw
hierarchy declarations across document roots, per-node parent/child fields, and
rich-table references. Malformed entries and declaration slots count toward
those limits, which are enforced before native-node decoding and reference
resolution. Persisted canonical views recheck the same block and outgoing-edge
bounds on every load. Ordered reconciliation uses indexed parent/child maps and
runs in O(nodes + edges), with explicit child order followed by native
collection order for inferred children.
...
Semantic admission and verified reads are therefore linear in the bounded
native graph, canonical graph, and overlay sizes. This deliberate replay cost
prevents a hash-correct but semantically fabricated node, range, scholarly
anchor, or metadata value from being accepted. Any future cache must be keyed
only by immutable product and configuration identities and must preserve the
